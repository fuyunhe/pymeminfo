...
def load_sections(map_file):
    ...
    for line in map_file:
        ...
        if m is not None:  # start of a new section
            ...
            continue
        #.ilm_code       0x0000000080010000     0x9124 load address 0x00000000010002c4
        RE_SECTION_HEADER_LOADADDR = r"(?P<name>[^ ]+) +0x(?P<address>[\da-f]+) +0x(?P<size>[\da-f]+) load address 0x(?P<loadaddr>[\da-f]+)$"
        m = re.match(RE_SECTION_HEADER_LOADADDR, line)
        if m is not None:  # start of a new section
            section = {
                "name": m.group("name"),
                "address": int(m.group("address"), 16),
...
def print_summary(memory_config, sections):
    ...
    total_iram = memory_config["ilm"]["length"]
    total_dram = memory_config["dlm"]["length"]
    used_data = get_size(".data")
    used_bss = get_size(".bss")
    used_dram = used_data + used_bss
    used_iram = sum(get_size(s) for s in sections if s.startswith(".text"))
    flash_code = get_size(".flash.text")
    flash_rodata = get_size(".rodata")
...
def print_detailed_sizes(sections, key, header):
    ...
    for k in sizes:
        v = sizes[k]
        result[k] = {}
        result[k]["data"] = v.get(".data", 0)
        result[k]["bss"] = v.get(".bss", 0)
        result[k]["text"] = sum(t for (s,t) in v.items() if s.startswith(".text"))
        result[k]["flash_text"] = v.get(".flash.text", 0)
        result[k]["rodata"] = v.get(".rodata", 0)
    ...
    for k,v in sorted(s, key=return_total_size, reverse=True):
        ...
        if ":" in k:  # print subheadings for key of format archive:file
            sh,k = k.split(":")
        print("%24s %10d %6d %6d %10d %8d %7d %24s" % (k[:24],
                                                  v["data"],
                                                  v["bss"],
                                                  v["text"],
                                                  v["flash_text"],
...
def print_archive_symbols(sections, archive):
    interested_sections = [".data", ".bss", ".text", ".iram0.vectors", ".flash.text", ".rodata"]
    result = {}
    ...
    for section in sections.values():
        ...
        for s in section["sources"]:
            ...
            if s["object_file"] not in result :
                result[s["object_file"]] = {}
            if section_name not in result[s["object_file"]] :
                result[s["object_file"]][section_name] = {}
            result[s["object_file"]][section_name][s["sym_name"]] = s["size"]
    outputs = ''
    #for t in interested_sections:
    ...
    #        #print(("%s(%d)\n" % (key.replace(t + ".", ""), val)), end=' ')
    #        outputs += ("{}({})\n").format(key.replace(t + ".", ""), val);
    #        section_total += val
    #    #print("\nSection total:",section_total)
    #    outputs += ("\nSection total: {}").format(section_total)

    def return_header(elem):
        return elem[0]
    s = sorted(list(result.items()), key=return_header)

    outputs += ("{:>7}\x09{:>7}\x09{:>7}\x09{:>7}\x09{}\n").format("text", "rodata", "data", "bss", "filename")
    for k,v in s: ##sorted(s, key=return_total_size, reverse=True):
        size = {}
        for t in interested_sections:
            size[t] = 0
        for kk,vv in v.items():
            size[kk] = sum(vv.values())
        outputs += ("{:>7}\x09{:>7}\x09{:>7}\x09{:>7}\x09{}\n").format(size[".text"], size[".rodata"], size[".data"], size[".bss"], k)
